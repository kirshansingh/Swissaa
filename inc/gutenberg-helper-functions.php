...
function generateElementStyles($elementType, $id, $target) {
  ...
  // Basic variables setup based on type of element (block or component)
  ...
  elseif($elementType == 'component') {
    $spacingArray = get_sub_field('settings_spacing_rwd');
    $spacingMode = get_sub_field('settings_spacing_mode');
    $id = get_sub_field('settings_unique_id');
    $mobileSpacingArray = get_sub_field('settings_spacing_mobile');
    $smSpacingArray = get_sub_field('settings_spacing_sm');
    $mdSpacingArray = get_sub_field('settings_spacing_md');
    $lgSpacingArray = get_sub_field('settings_spacing_lg');
    $xlSpacingArray = get_sub_field('settings_spacing_xl');
    $maxWidthMode = get_sub_field('settings_width_mode');
    $maxWidth = get_sub_field('settings_width');
    $mobileMaxWidth = get_sub_field('settings_width_mobile');
    $smMaxWidth = get_sub_field('settings_width_sm');
    $mdMaxWidth = get_sub_field('settings_width_md');
    $lgMaxWidth = get_sub_field('settings_width_lg');
    $xlMaxWidth = get_sub_field('settings_width_xl');
    $bgColor = get_sub_field('settings_background_color');
    $settingsTextColorMode = get_sub_field('settings_text_color_mode');
    $settingsCustomTextColor = get_sub_field('settings_custom_text_color');
  }
  ...
  if($maxWidthMode == 'advanced') {
    if(!empty($mobileMaxWidth) || !empty($smMaxWidth) || !empty($mdMaxWidth) || !empty($lgMaxWidth) || !empty($xlMaxWidth)) $wrapStylesShouldBeGenerated = true;
...
function generateElementTags($elementType, $is_preview){
  $tags = '';
  if($elementType == 'block') {
  ...
  if($elementType == 'component') {
    $bgImage = get_sub_field('settings_background_image');
    $animEnabled = get_sub_field('settings_enable_animations');
    $animType = get_sub_field('settings_animation_type');
    $animEasing = get_sub_field('settings_easing');
    $animDuration = get_sub_field('settings_duration');
    $animDelay = get_sub_field('settings_delay');
    $animOffset = get_sub_field('settings_offset');
    $groupName = get_sub_field('settings_group_name');
  }
...
function generateElementClasses($mode, $is_preview){
  $classes = '';

  $bgImage = get_uni_field('settings_background_image', $mode);
  $bgPosition = get_uni_field('settings_background_position', $mode);
  $bgRepeat = get_uni_field('settings_background_repeat', $mode);
  $bgSize = get_uni_field('settings_background_size', $mode);
  $bgAttachment = get_uni_field('settings_background_attachment', $mode);
  $visibility = get_uni_field('settings_visibility', $mode);
  $settingsTextColorMode = get_uni_field('settings_text_color_mode', $mode);
  $settingsDefaultTextColor = get_uni_field('settings_default_text_color', $mode);
  $textAlignMode = get_uni_field('settings_textalign_mode', $mode);
  $textAlign = get_uni_field('settings_textalign', $mode);
  $textAlignMobile = get_uni_field('settings_textalign_mobile', $mode);
  $textAlignSM = get_uni_field('settings_textalign_sm', $mode);
  $textAlignMD = get_uni_field('settings_textalign_md', $mode);
  $textAlignLG = get_uni_field('settings_textalign_lg', $mode);
  $textAlignXL = get_uni_field('settings_textalign_xl', $mode);
...
// Get ACF field data based on mode variable - block/component
function get_uni_field($fieldName, $mode) {
  if($mode == 'block') {
...
function the_uni_field($fieldName, $mode) {
  if($mode == 'block') {
